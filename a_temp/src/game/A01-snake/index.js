...
import { initialState as initSnake } from '@/reducers/snake';
...
const snakePosAndFailToTablePos = (failPos) => {
  let tablePos = initialState;
  for (let i = 0, l = failPos.length; i < l; ++i) {
    tablePos = tablePos.setIn([failPos[i][0], failPos[i][1]], 1);
  }
  return tablePos;
};

const gameFailAnimation = (cb) => {
  const snake = store.getState().get('snake');
  const [snakeHeadCol, snakeHeadRow] = [
    snake.get(snake.size - 1).get(0),
    snake.get(snake.size - 1).get(1),
  ];
  let col = 0;
  let row = 0;
  if (snakeHeadCol === 19) {
  ...
  for (let i = 0; i < failTable.length; i++) {
    failTable[i][0] += col + snakeHeadCol;
    failTable[i][1] += row + snakeHeadRow;
  }
  store.dispatch(setTable(snakePosAndFailToTablePos(failTable)));
  for (let i = 0; i < 4; i++) {
    setTimeout(() => {
      store.dispatch(setBlank());
      setTimeout(() => {
        store.dispatch(setTable(snakePosAndFailToTablePos(failTable)));
        if (i === 3) {
          store.dispatch(setBlank());
